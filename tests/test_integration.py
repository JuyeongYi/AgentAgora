...
@pytest.fixture
def runtime():
    """Minimal message-channel runtime: just InstanceRegistry + Dispatcher.

    The original v1 fixture constructed the full FastMCP app (which required
    the KV store + schema registry); v3 M0 removes those, and these tests only
    exercise dispatch/wait/register, so we instantiate the two pieces directly.
    """
    inst_reg = InstanceRegistry()
    disp = Dispatcher(inst_reg, default_timeout_ms=2000)
    return inst_reg, disp
