...
async def test_conversation_id_inherited_across_multi_hop_chain(setup):
    ...
    await dispatcher.dispatch(source="Inst2", target="Inst3", payload={"b": 2}, in_reply_to=m1["id"])
    m2 = (await dispatcher.wait("Inst3", timeout_ms=200))[0]
    await dispatcher.dispatch(source="Inst3", target="Inst1", payload={"c": 3}, in_reply_to=m2["id"])
...
async def test_target_inbox_depth_after_reflects_actual_queue_state_post_dispatch(setup):
    _, _, dispatcher = setup
    res1 = await dispatcher.dispatch(source="Inst1", target="Inst2", payload={"i": 1})
    assert res1["target_inbox_depth_after"]["Inst2"] == 1
    res2 = await dispatcher.dispatch(source="Inst1", target="Inst2", payload={"i": 2})
