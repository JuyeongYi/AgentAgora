...
- 워커 항목은 `{"r": [패턴], "w": [패턴]}` — `r`은 다운로드 허용 패턴, `w`는 업로드
  허용 패턴. 읽기/쓰기를 독립 지정한다.
- 패턴은 **gitignore 규칙과 동일한 시맨틱**으로 매칭한다 — `pathspec` 라이브러리
  (`gitwildmatch`). 와일드카드(`*.py`)·`**`·`!` negation·순서 last-match-wins 지원.
  공유/수신되는 파일명이 그 패턴 집합에 매치되면 해당 op 허용. `["*"]`=전체 허용,
  `[]`=전부 거부. `pathspec`은 `pyproject.toml` 의존성에 추가한다(작고 순수 파이썬·
  무의존 라이브러리).
- 워커 항목에서 차원이 누락되면 비대칭 기본값을 적용한다 — `r` 누락 → `["*"]`
  (다운로드 전체 허용), `w` 누락 → `[]`(업로드 전체 거부). 다운로드는 기본 개방,
  업로드는 기본 차단(safe-by-default — 위험한 쪽을 명시 grant하게).
- `workers` — 명시 등재 워커별 정책.
- `fallback` — 선택. `workers`에 없는 워커에 적용되는 기본 정책. `fallback`이 없으면
  미등재 워커는 무제한(`r`·`w` 모두 전체 허용).
- **파일(`file-policy.json`)이 아예 없으면** `FilePolicy` 비활성 → 전원 무제한.

업로드(`share_file`·`POST /files`)는 `w` 패턴이, 다운로드(`fetch_file`·`GET /files`)는
`r` 패턴이 게이트한다 — 읽기·쓰기가 각각 독립 제어된다.

`FilePolicy` API:
- `active: bool` — 설정 파일 존재 여부.
- `load_json(text)` — 파싱·제자리 교체. 잘못된 구조는 `AgoraError`.
- `can_upload(worker_id, file_name) -> bool` — 정책 해석(워커 항목 → `fallback` →
  무제한/비활성). 무제한이면 True. 아니면 `file_name`(basename)이 워커 `w` 패턴에
  gitignore 시맨틱으로 매치되면 True.
- `can_download(worker_id, file_name) -> bool` — 같은 해석, `r` 패턴으로 판정.
- `snapshot() -> dict` — 현재 정책 조회용(admin GET).

패턴 매칭은 파일 basename(예 `report.py`)에 대해 한다. 차원별 패턴을
...
- **`agora.fetch_file(file_id, dest_path)`** — 호출자 instance_id 해석 →
  `file_policy.can_download(caller, name)` 검사. 거부 시 `AgoraError("file_download_denied")`.
  통과 시 스토어의 `<file_id>`를 `dest_path`로 복사. `file_id`가 없으면
  `AgoraError("unknown_file")`.
...
- **`POST /files`** — 바이트 업로드. 요청자 식별은 `X-Agora-Instance-Id` 헤더
  (auto-register와 동일) → `file_policy.can_upload` 검사. 파일명은 헤더 또는 멀티파트.
  `max_file_bytes` 초과 거부. 스토어에 저장 후 핸들 JSON 반환.
- **`GET /files/<file_id>`** — 다운로드. 요청자 식별은 `X-Agora-Instance-Id` 헤더 →
  `file_policy.can_download` 검사. 통과 시 스토어 파일을 바이트로 반환. 미존재 404,
  권한 거부 403.
...
파일 공유는 응집도 높은 한 기능이다 — 스토어·`FilePolicy`·HTTP 엔드포인트·GC가
서로 맞물려 동작한다. 구현 단위를 쪼개면 조각마다 설계가 흔들릴 수 있으므로 한
플랜으로 구현하며, 태스크를 다음 순서로 둔다:

1. 파일 스토어 — `.agentagora/files/` + `files` 영속 테이블·마이그레이션, `max_file_bytes`.
2. `file_share` 빌트인 스키마(`default_schemas.jsonl` 등재 + startup permanent 등록).
3. `agora.share_file`·`agora.fetch_file` MCP 도구.
4. `pathspec` 의존성 + `FilePolicy` 클래스(JSON 로드·gitignore 패턴 매칭·`can_upload`·
   `can_download`·`snapshot`).
5. share/fetch 도구에 `FilePolicy` 게이트 배선 + startup `file-policy.json` 로드.
6. HTTP `POST /files`·`GET /files/<id>`(`file_routes.py`) + 앱 와이어링.
7. `/admin/file-policy` 엔드포인트.
8. `Sweeper.file_gc_sweep` + GC 루프 배선.
