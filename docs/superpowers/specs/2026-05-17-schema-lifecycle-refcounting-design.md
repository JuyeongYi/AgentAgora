...
**재시작 동작 — 런타임 스키마 복원 중단.** 현재 startup(`__main__.py:91`)은
`persistence.restore_schemas()`로 SQLite의 등록 스키마를 무조건 복원한다. ref-counting
하에서 이 복원은 고아 ref를 만든다 — 복원된 스키마의 holder(인스턴스·봇)는 재시작 후
복원되지 않아(`BotRegistry`는 재시작 복원 안 함, 세션도 죽음) 그 ref가 영원히 해제되지
않는다. 따라서 **런타임 스키마 복원을 중단한다** — startup은 `schemas.jsonl`의 빌트인만
로드하고 `restore_schemas()` 호출을 제거한다. 봇·워커는 재접속 시 스키마를 재등록한다
(봇은 `register_bot` 인라인 `schemas=`로 이미 재등록, 워커는 `agora.register_schema`
재호출). SQLite `schemas` 테이블은 그대로 두되 audit 기록으로만 쓰이고 startup이 읽지
않는다. `BotRegistry` 미복원 + channel-receive-finalize 클린 스타트 철학과 일관된다.
...
2. `schema_conflict` 빌트인 — `default_schemas.jsonl` 추가 + startup permanent 재등록.
   더불어 startup(`__main__.py`)의 `restore_schemas()` 호출 제거 — 런타임 스키마 복원
   중단(§3 재시작 동작).
