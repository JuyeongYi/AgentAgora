...
- 스크립트 생성 옵션 — `server_launcher`/`run_all`(manifest 불린, 기본 true). server_launcher=`run-server.bat`/`run-server.sh`, run_all=`run-all.ps1`(wt.exe 탭→Start-Process)/`run-all.sh`(**zellij 전용** — 세션 안이면 `zellij action new-tab --cwd … -- …`로 각 워커를 새 탭으로, 세션 밖이면(`ZELLIJ` 미설정) 임시 layout으로 새 zellij 세션을 띄워 자신을 재실행; tmux 자동감지를 제거해 zellij 세션 안 tmux 중첩을 막음). run-all 동작: (server_launcher일 때) 서버 기동→포트 대기→`.mcp.json` 있는 하위 워커 순차 기동; server_launcher가 꺼지면 `include_server=False`로 서버 기동을 생략하고 워커만 띄운다(서버 외부 기동 가정).
