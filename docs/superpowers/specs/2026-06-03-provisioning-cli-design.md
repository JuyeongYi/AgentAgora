...
- 스크립트 생성 옵션 — `server_launcher`/`run_all`(manifest 불린, 기본 true). server_launcher=`run-server.bat`/`run-server.sh`, run_all=`run-all.ps1`(Windows)/`run-all.sh`(POSIX) — **둘 다 zellij 전용**(OS 무관 통일; Windows 네이티브 zellij ≥0.44.0 필요, 미설치 시 안내 후 종료): 세션 안이면 `zellij action new-tab --cwd … -- …`로 각 워커를 새 탭으로(Windows는 `-- cmd /c run.bat`, POSIX는 `-- bash run.sh`), 세션 밖이면(`ZELLIJ`/`$env:ZELLIJ` 미설정) 임시 layout으로 새 zellij 세션을 띄워 자신을 재실행(`wt.exe`/tmux 자동감지 없음 — OS 무관 단일 멀티플렉서). run-all 동작: (server_launcher일 때) 서버 기동→포트 대기→`.mcp.json` 있는 하위 워커 순차 기동; server_launcher가 꺼지면 `include_server=False`로 서버 기동을 생략하고 워커만 띄운다(서버 외부 기동 가정).
- 런처 포트·바인드 — `_server_url_parts(server_url)`로 결정해 템플릿 `{{PORT}}`·`{{BIND_OPT}}`를 치환. 호스트가 127.0.0.1/localhost면 기본(로컬), 비-로컬이면 `--bind-host 0.0.0.0`(분산 셋업: 다른 PC 접속 허용). agora-init은 서버를 직접 기동하지 않고 스크립트만 생성.
