...
CSV 헤더(열 라벨)와 데이터 행 라벨은 **정규식 패턴**이다. 서버는 `re.fullmatch`로
인스턴스 id 전체와 대조한다 — 패턴이 id 전체와 일치해야 매칭된다.

- **정확한 인스턴스명** — `pm`, `Coder1` 처럼 정확한 이름은 자명한 정규식이므로
  해당 인스턴스 하나에만 fullmatch된다.
- **역할군 패턴** — `coder-.*` 같은 패턴 한 줄로 `coder-A`, `coder-B` 등 여러
  인스턴스를 동시에 커버할 수 있다.
- **catch-all** — `.*`는 모든 인스턴스에 fullmatch된다. 기존 `*` 와일드카드는
  유효하지 않은 정규식이므로 폐지됐다. catch-all은 반드시 `.*`로 쓴다.
- **다중 패턴 매칭** — 여러 패턴이 동시에 매칭되면 **max weight**(권한이 높은 쪽)를
  적용한다.

`.*` catch-all을 행·열에 모두 넣으면 미등재 인스턴스에 대한 폴백 weight를 지정할 수 있다.

- **`.*` 데이터 행** — 다른 패턴 행에 매칭되지 않는 `to`(수신자)에 적용되는 폴백 행.
- **`.*` 헤더 열** — 다른 패턴 열에 매칭되지 않는 `from`(발신자)에 적용되는 폴백 열.
- **`_weights[".*"][".*"]`** — 발신자·수신자 둘 다 다른 패턴에 매칭되지 않는 catch-all.

폴백 해석 순서:

1. `to` id에 fullmatch되는 행 중 가장 높은 weight를 가진 행을 사용.
2. 매칭 행이 없으면 `.*` 행으로 폴백.
3. 해당 행에서 `from` id에 fullmatch되는 열 셀 값을 사용 (다중 매칭이면 max weight).
4. 매칭 열이 없으면 같은 행의 `.*` 열로 폴백. 그것도 없으면 `0`.

`.*`가 **없는** CSV는 strict whitelist다 — 어떤 패턴에도 매칭되지 않는 `from`·`to`는
모두 거부된다.

예시 — 허브·스포크 구성에 `.*` catch-all 활용:
...
헤더(`from` 패턴 목록): `.*`, `pm`, `coder`, `reviewer`. 데이터 행 라벨(`to` 패턴):
`.*`, `pm`, `coder`, `reviewer`. `.*`를 포함하면 (N+1)×(N+1) 정사각이 된다.
...
`matrix` 필드는 `{to: {from: weight}}` 구조다. `.*` catch-all 행·열이 있으면 포함된다.
...
매트릭스 상태는 [`/dashboard`](dashboard.md#comm-matrix-방향-그래프)에서 SVG 방향
그래프로 실시간 확인된다 — 노드=인스턴스, 화살표=허용 dispatch 방향, 라벨=weight.
원형 layout, comm-matrix가 비활성이면 "all-allow" 안내.
...
- [`docs/superpowers/specs/2026-05-18-comm-matrix-fallback-design.md`](superpowers/specs/2026-05-18-comm-matrix-fallback-design.md) — catch-all 폴백 설계 문서 (초기 `*` 와일드카드 → `.*` 정규식 전환 결정 포함)
- [`docs/usage-guide.md`](usage-guide.md) — 전체 워커·봇·매트릭스 사용 가이드
- [`docs/dashboard.md`](dashboard.md) — 대시보드 SVG 그래프 + 운영자 dispatch 채널
