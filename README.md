...
각 `Inst*/` 안에서 `run.bat`(채널 모드 기동 스크립트)을 실행하면 그 디렉토리의
`.mcp.json` 헤더로 서버에 자동 등록된다. 각자 별도 터미널에서 띄우면 N개
에이전트가 동시에 돈다.

워커는 `agora-channel` 어댑터를 통한 **채널 모드**([`docs/channel-mode.md`](docs/channel-mode.md))로
메시지를 받는다 — 서버 push로 워커를 깨우며, 블록된 도구 콜 없이 진짜 idle 상태로 기다린다.

새 워커를 추가하려면 디렉토리·`CLAUDE.md`·`.mcp.json`(2-서버)·`run.bat`을 손으로
챙겨야 하는데, 그걸 한 줄로 줄이는 Claude Code 플러그인이 별도로 있다 —
[`plugin/cc-agora-ops/`](plugin/cc-agora-ops/). 플러그인의 `/cc-agora-ops:agora-spawn`이 채널 모드 번들을 자동 생성한다.
...
- **`agora.flush(from_sources=None, by_conversation=None, sort="fifo")`** —
  현재 큐에 쌓인 명령을 즉시 드레인. 논블로킹 — 큐가 비어 있으면 빈 배열 반환. 워커·봇 모두 사용.
...
| `X-Agora-Wait-Timeout-Ms`     | (제거됨 — blocking wait 도구 폐기 시 함께 제거. `agora.flush`는 timeout 없음.) |
