# AgentAgora 플러그인 — manual smoke test 시나리오
...
- cc-agora 플러그인이 Claude Code에 등록되어 있다 (`~/.claude/plugins/cc-agora` symlink 또는 marketplace 설치). 선택적으로 역할 페르소나 플러그인(`cc-agora-coder` 등)을 추가로 활성화할 수 있다 — 아래 §0.1 참조.
- 작업용 빈 디렉토리 — 예: `C:/AgoraTeam/` — 가 비어 있고 git 추적 밖이다.
- (Windows) `wt.exe`가 PATH에 있다. 없으면 `--launch=auto` 시나리오는 manual 강등 동작을 검증한다.

### 0.1 페르소나 플러그인 활성화 (선택)

워커 디렉토리의 `.claude/settings.local.json`에서 `enabledPlugins`에 원하는 역할 플러그인을 추가한다. `agora-spawn` 또는 `agora-spawn-team`이 spawn 시 생성하는 `.claude/settings.local.json`에 이 설정을 미리 넣어두면 워커 기동 시 자동 적용된다.
...
사용 가능한 페르소나 플러그인: `cc-agora-orchestrator`, `cc-agora-coder`, `cc-agora-reviewer`, `cc-agora-tester`, `cc-agora-writer`, `cc-agora-planner`, `cc-agora-general`.

### 0.2 spawn 산출물

`agora-spawn`은 워커 디렉토리에 다음 4개 파일을 생성한다:

```
<WorkerDir>/
  CLAUDE.md                      # 워커 역할 설명 (thin — 페르소나 내용 없음)
  .mcp.json                      # AgentAgora MCP 서버 연결 설정
  run.bat                        # 채널 모드로 claude를 기동하는 배치 파일
  .claude/settings.local.json    # enabledPlugins 포함 로컬 설정
```

페르소나 내용(미션·응답 규약·역할별 지식)은 각 페르소나 플러그인의 `skills/persona/SKILL.md`가 담당한다 — CLAUDE.md에 중복 기술하지 않는다.
...
1. orchestrator 디렉토리 셋업 — 별도 터미널에서:
   ...
   ```
   /cc-agora-ops:agora-spawn Orch1 orchestrator "팀 작업 분배 PM."
   /cc-agora-ops:agora-spawn Coder1 coder "React 컴포넌트 코딩 담당."
   ```
...
- `C:/AgoraTeam/Orch1/` — CLAUDE.md + .mcp.json + run.bat + `.claude/settings.local.json` 4개 존재.
- `C:/AgoraTeam/Coder1/` — CLAUDE.md + .mcp.json + run.bat + `.claude/settings.local.json` 4개 존재.
- 서버 로그에 두 인스턴스 등록 이벤트 + dispatch + flush 짝.
...
1. orchestrator 디렉토리에서:
   ...
   ```
   /cc-agora-ops:agora-spawn-team <plugin-root>/templates/team.json.example --dir=C:/AgoraTeam --launch=auto
   ```
...
- 각 디렉토리에 4개 파일 (CLAUDE.md + .mcp.json + run.bat + `.claude/settings.local.json`).
...
1. orchestrator 디렉토리에서 `--force` 없이 spawn 재시도:
   ...
   ```
   /cc-agora-ops:agora-spawn Coder1 coder "다른 설명."
   ```

2. `--force`를 붙여 재시도:

   ```
   /cc-agora-ops:agora-spawn Coder1 coder "다른 설명." --force
   ```
...
1. orchestrator에서:
   ...
   ```
   /cc-agora-ops:agora-spawn Phantom1 phantom "미정의 role 테스트."
   ```
...
- `Phantom1/CLAUDE.md` + `Phantom1/.mcp.json` + `Phantom1/run.bat` + `Phantom1/.claude/settings.local.json` 4개 생성됨.
- `Phantom1/.claude/settings.local.json`에 `cc-agora-general@agentagora: true`가 활성화되어 있다.
- CLAUDE.md는 fallback role로 설명을 담는다. 페르소나 지식은 `cc-agora-general` 플러그인이 담당한다.
