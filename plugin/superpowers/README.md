...
improver (improvement-review)
   │
   └──▶ planner ↻  (개선 발견 시 재순환)  /  종료 (개선 없거나 유저 거부)
```

각 엣지는 `comm-matrix.csv`로 게이팅된다. 대상이 불명확한 위임은 라우팅 봇이
`agora.find`로 resolve한다.
...
| `superpowers-implementer` | `plugin/superpowers/superpowers-implementer/` | TDD + executing-plans + git 워크트리 + 브랜치 마무리 |
...
운영자 세션에서 `/cc-agora-ops:agora-spawn`으로 6개 페르소나 워커를 생성한다.
